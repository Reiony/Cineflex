  ...
  return (
    <PaginaAssentos>
      ...
      <CaixaAssentos>
        {sumariosessao.seats.map((item, index) => (
          <BolaAssento
            ...
            name={item.name}
            onClick={() => VerificaAssento(item.name, item.isAvailable)}
            key={index}
          >
            <h4>{item.name}</h4>
          </BolaAssento>
      ...
      <FooterFilme>
        <Borda>
          <img
            src={sumariosessao.movie.posterURL}
            alt={sumariosessao.movie.posterURL}
          />
        </Borda>
        <ResumoFilmeSelecionado>
          <h3>{sumariosessao.movie.title}</h3>
          <h3>
            {sumariosessao.day.weekday} - {sumariosessao.name}
          </h3>
        </ResumoFilmeSelecionado>
      </FooterFilme>
    </PaginaAssentos>
...
const BolaAssento = styled.div`
  ...
  background: ${(props) =>
    props.isavailable
      ? props.assentosescolhidos.includes(props.name)
        ? '#1AAE9E'
        : '#C3CFD9'
      : '#FBE192'};
  border: 1px solid
  ...
  h4 {
    ...
    text-align: center;
    letter-spacing: 0.04em;
    color: ${(props) => (props.isavailable ? '#FFFFFF' : '#000000')};
  }
`;
